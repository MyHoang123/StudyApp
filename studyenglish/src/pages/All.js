...
function All({handleCheckResuilt}) {
    ...
    const getVocabulary = async (page) => {
        try {
          const response = await fetch(`http://localhost:8080/api/v12/getallvocabulary?Index=${page}`);
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
          const result = await response.json();
          setVietnamese(result.Vietnamese)
          setEnglishs(result.English)
        } catch (error) {
          alert(error)
        }
      }
    const handleClickNext = () => {
        getVocabulary(Page + 1)
        setPage(prev => prev += 1)
      }
